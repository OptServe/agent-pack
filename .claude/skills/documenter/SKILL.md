...
This is a lighter-weight pass than what the slice-5 hook will eventually automate on `PostToolUse`. Both layers complement: hook catches drift at the moment of edit; `/checkpoint` no-args catches accumulated drift the user wants to audit on demand.
...
- **`/feature-merge`** (slice 6) loads this skill for the doc-reconciliation gate before merge. The reconciliation is a "final-pass" version of Pattern D plus a summary-authoring step.
