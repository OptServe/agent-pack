...
- A one-line prompt: "Review the plan; when ready, change its front-matter `status` from `draft` to `approved` and run `/build`."

Do not auto-approve the plan. Approval is the user's decision and is the gate `/build` checks (`/build` reads the plan's front-matter `status` directly, so the user only needs to edit the plan file — state.md's `Phase` is descriptive).
