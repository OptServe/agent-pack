...
- Any ADRs written.
- The branch name.
- A one-line prompt: "Review the spec; when ready, change its front-matter `status` from `draft` to `approved` and run `/plan`."

Do not auto-approve the spec. Approval is the user's decision and the gate `/plan` checks.
